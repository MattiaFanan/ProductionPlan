...
class PPMultiCommodity:
    ...
    def build_model(self):
        ...
        # variables
        model.Production = Var(model.Weeks, model.Weeks, domain=NonNegativeReals)
        model.SetUp = Var(model.Weeks, domain=Binary)
        model.Stock = Var(model.Weeks, model.Weeks, domain=NonNegativeReals)
        # params
        ...
        # constraints
        model.pc = Constraint(model.Weeks, model.Weeks, rule=self._production_rule)
        model.sc = Constraint(model.Weeks, model.Weeks, rule=self._stock_rule)
        return model
